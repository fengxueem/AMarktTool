...
from config import AK_API_STOCK_DF_NAME, AK_API_STUPID_ST
...
class MagicNineModel:
    ...
    def __init__(self) -> None:
        ...
        shanghai_stock_df = shanghai_stock_df[~shanghai_stock_df[AK_API_STOCK_DF_NAME].str.contains(AK_API_STUPID_ST)]
        # 创建一个新的列表，只保留原先需要的属性
        new_shanghai_stock = [{TABLE_STOCK_CODE: item[1], TABLE_STOCK_NAME: item[2]} for item in shanghai_stock_df.values]

        # 获取深 A 股实时行情数据
        shenzhen_stock_df = ak.stock_sz_a_spot_em()
        shenzhen_stock_df = shenzhen_stock_df[~shenzhen_stock_df[AK_API_STOCK_DF_NAME].str.contains(AK_API_STUPID_ST)]
        new_shenzhen_stock = [{TABLE_STOCK_CODE: item[1], TABLE_STOCK_NAME: item[2]} for item in shenzhen_stock_df.values]
